...
| — | `2 → 1 → 7` | TLS cert خودکار (Let's Encrypt) |
...
- **TLS خودکار:** گزینهٔ ۱ = Let's Encrypt از طریق **nginx** روی `:80` (ACME + صفحهٔ فیک). بعد از پوینت DNS، certbot با webroot سرت می‌گیرد و در `/etc/gost/certs/` برای listenerهای GOST می‌گذارد. اگر GOST قبلاً `:80` را گرفته باشد، decoy به nginx منتقل می‌شود تا gost قطع نشود.
...
| — | `2 → 1 → 7` | Auto TLS cert (Let's Encrypt) |
...
**Auto TLS:** wizard option 1 = Let's Encrypt via **nginx** on `:80` (ACME + decoy page). After you point DNS, certbot uses webroot and copies certs to `/etc/gost/certs/` for GOST listeners. If GOST already held `:80`, decoy is moved to nginx so gost stays up.
